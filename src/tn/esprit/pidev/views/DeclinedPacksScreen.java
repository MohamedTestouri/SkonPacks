...
public class DeclinedPacksScreen extends  Form{
    Form current;
    PackService packService = new PackService();
    ArrayList<Pack> packArrayList = new ArrayList<>();
    public DeclinedPacksScreen(Form previous) {
        /* *** *CONFIG SCREEN* *** */
        current = this;
        setTitle("Declined Packs");
        setLayout(BoxLayout.y());
        /* *** *YOUR CODE GOES HERE* *** */
        packArrayList = packService.showMyPacks("packs/declined"); // Only declined packs
        ...
        for (Pack pack : packArrayList){
            ...
            multiButton.addActionListener(evt -> {new ShowDetailPackScreen(current, pack).show();});
            add(multiButton);
        }
        /* *** *SEARCHBAR* *** */
